...
def get_vision_sensor_image():
	...
	# Getting the object handle of the vision sensor
	return_code, vision_sensor = sim.simxGetObjectHandle(client_id, 'Vision_sensor', sim.simx_opmode_blocking)

	# Getting the image and the resolution from the coppeliasim server
...
def transform_vision_sensor_image(vision_sensor_image, image_resolution):
	...
	# Converting the list intp numpy array and changing its dtype to uint8
	img = np.array(vision_sensor_image, dtype = np.uint8)

	# Reshaping the image from 1D array to 3D array by using the resolution of the image returned by the server
	img = img.reshape((image_resolution[0], image_resolution[1], 3))

	# Converting BGR to RGB
	img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

	# Flipping image upside down
	transformed_image = np.flip(img, 0)
...
def send_data(client_id, maze_array):
	...
	# Creating an empty buffer array
	emptyArray = bytearray()

	# Flattening the mazeArray before sending to the server because only 1D arrays can be sent 
	mazeArrayFlat = []

	for item in maze_array:
		mazeArrayFlat.extend(item)

	# Sending the data to the server
	return_code, outInts, outFloats, outStrings, outBuffer = sim.simxCallScriptFunction(client_id, 'top_plate_respondable_1', 
		sim.sim_scripttype_customizationscript, 'receiveData', mazeArrayFlat, [], [], emptyArray, sim.simx_opmode_blocking)
